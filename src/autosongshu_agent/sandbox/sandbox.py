...
from ..config import SandboxConfig, ScopePolicy
from .utils import (
    SandboxError,
    build_unified_diff,
    coerce_text,
...
class PythonSandbox:
    ...
    def __init__(
        ...
        self.user_id = normalize_user_id(user_id or settings.default_user_id)
    ...
    def _resolve_in_session(self, relative_path: str) -> Path:
        ...
        if not candidate.is_relative_to(session_root):
            raise SandboxError(
                f"Sandbox path escapes session artifacts directory: {relative_path}"
            )
        return candidate
    ...
    def _wait_for_pending_workspace_update(
        self, target: Path, *, timeout_sec: float = _PENDING_WORKSPACE_UPDATE_WAIT_SEC
    ) -> None:
        ...
        while True:
            with self._shared_state_lock:
                record = self._pending_workspace_updates.get(key)
                pending_event = (
                    record.get("event") if isinstance(record, dict) else None
                )
            ...
            if pending_event is None:
                if target.is_file():
                    ...
                    time.sleep(
                        min(
                            _PENDING_WORKSPACE_UPDATE_POLL_SEC,
            ...
            if remaining <= 0:
                return
            pending_event.wait(
                timeout=min(_PENDING_WORKSPACE_UPDATE_POLL_SEC, remaining)
            )
    ...
    def _bootstrap_state_is_current(self) -> bool:
        ...
        return payload.get("packages") == list(
            self.settings.bootstrap_packages
    ...
    def _install_bootstrap_packages(self) -> None:
        packages = [
            item.strip() for item in self.settings.bootstrap_packages if item.strip()
        ]
        if not packages:
            return
    ...
    def _base_env(self) -> dict[str, str]:
        ...
        env["AUTOSONGSHU_SCOPE_ALLOWED_HOSTS"] = json.dumps(
            self.scope.allowed_hosts, ensure_ascii=False
        )
        env["AUTOSONGSHU_SCOPE_ALLOW_SUBDOMAINS"] = str(
        ...
        env["AUTOSONGSHU_SANDBOX_IGNORE_HTTPS_ERRORS"] = str(
    ...
    def status(
        self, include_packages: bool = False, package_limit: int = 50
    ) -> dict[str, Any]:
        with self._lock:
            self._ensure_directories()
            ...
            if include_packages and self.python_executable.exists():
                payload["packages"] = self.list_packages(limit=package_limit)[
                    "packages"
                ]
            return payload
    ...
    def write_file(self, path: str, content: str) -> dict[str, Any]:
        ...
        validate_raw_file_content(content, path=target)
        ...
        if use_text_replace:
            ...
            if lines:
                ...
                if expected_old_text and selected_text != expected_old_text:
                    raise SandboxError(
                        "expected_old_text did not match the selected line range."
                    )
                updated = "".join(
                    [*lines[: start_line - 1], new_text, *lines[end_line:]]
                )
                spans_entire_file = start_line == 1 and end_line == len(lines)
                if (
                    reject_full_file_replace
                    and spans_entire_file
                    and len(lines) > 1
                ...
                raise SandboxError(
                    "Cannot replace a non-existent line range in an empty file."
                )
        ...
        *,
        ...
        try:
            with self._lock:
                ...
                validate_raw_file_content(updated, path=target)

                diff_preview, diff_truncated = build_unified_diff(
                    original,
                    updated,
        ...
        try:
            with self._lock:
                ...
                for index, edit in enumerate(edits, start=1):
                    ...
                    validate_raw_file_content(next_updated, path=target)
        ...
        *,
        ...
        with self._lock:
            if not self.settings.allow_package_install:
                raise SandboxError(
                    "Sandbox package installation is disabled by configuration."
                )
            ...
            stdout_preview, stdout_truncated = truncate_text(
                result["stdout"], max_output_chars
            )
            stderr_preview, stderr_truncated = truncate_text(
    ...
    def run_python(
        ...
        *,
        ...
        if code.strip():
            validate_raw_file_content(code, language_hint="python")
        ...
        if code.strip():
            script_file = (
                self.workspace_dir / ".runs" / f"inline-{int(time.time() * 1000)}.py"
            )
        else:
        ...
        with self._lock:
            ...
            runtime_hints = detect_runtime_hints(stdout_preview, stderr_preview)
            self._log_activity(
                "run_python",
                {
            ...
            response_payload = {
                ...
                "duration_sec": result["duration_sec"],
                "script_path": str(script_file),
                "relative_script_path": script_file.relative_to(
                    self.workspace_dir
                ).as_posix(),
                "created_inline_script": created_inline_script,
                "args": args,
