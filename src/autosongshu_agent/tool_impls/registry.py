...
def _wrap_registered_tool(
    ...
    *,
    ...
    # We strip `runtime` from the signature so AgentScope won't expose it.
    ...
    def wrapped(*args: Any, **kwargs: Any) -> ToolResponse:
        # Bind the exposed arguments to get a clean dict of call parameters
        ...
        if execution_policy.requires_approval:
            ...
            print(f"Arguments: {json.dumps(call_arguments, ensure_ascii=False, indent=2, default=str)}")
...
class ToolRegistry:
    ...
    def create_group(self, name: str, description: str, active: bool | Callable[[PentestRuntime], bool] = True, notes: str = "") -> None:
        self.groups[name] = ToolGroupInfo(name, description, active, notes)

    def register(self, group_name: str, **policy_kwargs: Any) -> Callable[[Callable[..., ToolResponse]], Callable[..., ToolResponse]]:
    ...
    def register_all_to_toolkit(self, toolkit: Toolkit, runtime: PentestRuntime) -> None:
        ...
        for tool_info in self.tools:
            wrapped = _wrap_registered_tool(tool_info.func, runtime, policy=tool_info.policy)
