...
class CDPBrowserSession:
    def __init__(
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="autosongshu-browser"
        )
    ...
    def _serialize_cdp_request_record(
        ...
        request["post_data"] = truncate_text(request.get("post_data"), body_chars)
        body_preview = truncate_text(record.get("response_body_preview"), body_chars)
        return {
    ...
    def _capture_cdp_request_post_data(
        self, request_id: str, record: dict[str, Any]
    ) -> None:
    ...
    def _capture_cdp_response_body(
        self, request_id: str, record: dict[str, Any]
    ) -> None:
        if self.cdp_session is None:
            return
        ...
        content_type = (
            ...
            or (record.get("response_extra_info") or {})
            .get("headers", {})
            .get("content-type")
        )
        if not is_textual_resource(record.get("resource_type"), content_type):
    ...
    def _on_cdp_request_will_be_sent(self, event: dict[str, Any]) -> None:
        ...
        record["timestamps"]["request"] = now_iso()
        record["request"] = {
            "url": request.get("url"),
    ...
    def _on_cdp_request_extra_info(self, event: dict[str, Any]) -> None:
        ...
        record["request_extra_info"] = {
            ...
            "site_has_cookie_in_other_partition": event.get(
    ...
    def _on_cdp_response_received(self, event: dict[str, Any]) -> None:
        ...
        record["frame_id"] = event.get("frameId") or record.get("frame_id")
        record["loader_id"] = event.get("loaderId") or record.get("loader_id")
        record["timestamps"]["response"] = now_iso()
        record["response"] = {
    ...
    def _on_cdp_loading_finished(self, event: dict[str, Any]) -> None:
        ...
        record["timestamps"]["finished"] = now_iso()
        response = record.setdefault("response", {})
    ...
    def _on_cdp_loading_failed(self, event: dict[str, Any]) -> None:
        ...
        record["timestamps"]["failed"] = now_iso()
    ...
    def _run_in_browser_thread(self, function: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if (
            ...
            and current_thread_id == self._browser_thread_id
        ):
            return function(*args, **kwargs)

        future = self._executor.submit(
            self._call_in_browser_thread, function, *args, **kwargs
        )
    ...
    def _navigate_impl(self, url: str) -> dict[str, Any]:
        page = self._ensure_page()
        target_url = self.scope.assert_in_scope(
            url, current_url=self._current_url_impl()
        )
        response = page.goto(target_url, wait_until="domcontentloaded")
        try:
            page.wait_for_load_state(
                "networkidle", timeout=min(self.settings.timeout_ms, 5000)
            )
        except Exception:
            pass
    ...
    def _wait_for_load_state_impl(
        self, state: str = "networkidle", timeout_ms: int | None = None
    ) -> dict[str, Any]:
        page = self._ensure_page()
        effective_timeout = (
            timeout_ms if timeout_ms is not None else self.settings.timeout_ms
        )
    ...
    def wait_for_load_state(
        ...
        return self._run_in_browser_thread(
            self._wait_for_load_state_impl, state, timeout_ms
        )
    ...
    def wait_for_selector(
        ...
        return self._run_in_browser_thread(
            self._wait_for_selector_impl, selector, state, timeout_ms
        )
    ...
    def list_forms(
        self, max_forms: int = 40, max_inputs: int = 40
    ) -> list[dict[str, Any]]:
        return self._run_in_browser_thread(
    ...
    def _storage_snapshot_impl(self) -> dict[str, Any]:
        snapshot = self._snapshot_impl(
            max_chars=1000, max_forms=0, max_inputs=0, max_links=0, storage_items=50
        )
        return {
            "url": snapshot["url"],
    ...
    def get_response_bodies(
        self, limit: int = 40, url_contains: str = ""
    ) -> list[dict[str, Any]]:
        return self._run_in_browser_thread(
            self._get_response_bodies_impl, limit, url_contains
        )

    def _get_response_bodies_impl(
        self, limit: int = 40, url_contains: str = ""
    ) -> list[dict[str, Any]]:
    ...
    def get_cdp_requests(
        ...
        return self._run_in_browser_thread(
            self._get_cdp_requests_impl, limit, url_contains, body_chars
        )
    ...
    def _get_cdp_requests_impl(
        ...
        records = [
            self._serialize_cdp_request_record(
                self.cdp_request_details[request_id], body_chars=body_chars
            )
    ...
    def _analyze_page_resources_impl(
        ...
        if current_item is not None:
            ...
            current_item["resource_type"] = (
                current_item.get("resource_type") or "document"
            )
        ...
        for record in cdp_records:
            ...
            item["resource_type"] = record.get("resource_type") or item.get(
                "resource_type"
            )
        ...
        for entry in inventory.get("performance_resources") or []:
            ...
            item["resource_type"] = item.get("resource_type") or entry.get(
                "initiator_type"
            )
            item["performance"] = {
        ...
        resource_type_counts = Counter(
            str(item.get("resource_type") or "unknown") for item in resources
        )
        source_counts = Counter(
            source for item in resources for source in item.get("sources") or []
        )
        ...
        return {
            ...
            "summary": {
                ...
                "performance_resource_count": len(
                    inventory.get("performance_resources") or []
                ),
