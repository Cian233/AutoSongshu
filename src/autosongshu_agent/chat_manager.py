...
from .knowledge_store import KnowledgeBaseDraft, KnowledgeBaseUpdateDraft, KnowledgeDocumentDraft, KnowledgeStore
from .memory import LayeredConversationMemory, build_memory_transcript_payload, completed_messages_after_anchor
...
def _extract_absolute_urls(text: str) -> list[str]:
    matches = _ABSOLUTE_URL_PATTERN.findall(str(text or ""))
    urls: list[str] = []
    seen: set[str] = set()
    for raw in matches:
...
def _extract_recent_sandbox_script_paths(messages: list["ChatMessage"], limit: int = 6) -> list[str]:
    seen: set[str] = set()
    recent_paths: list[str] = []
    assistant_messages = [message for message in messages if message.role == "assistant" and message.status == "completed"]
    ...
    for message in reversed(assistant_messages[-24:]):
        ...
        for part in reversed(parts):
            ...
            raw_path = arguments.get("script_path") if name == "sandbox_run_python" else arguments.get("path")
...
def _message_payload_size(message: "ChatMessage") -> int:
    role = str(getattr(message, "role", "assistant"))
    content = normalize_message_content(getattr(message, "content", []) or [], role=role)
...
def _should_force_stream_flush(event: dict[str, Any]) -> bool:
    if bool(event.get("last")):
        return True

    for block in event.get("blocks") or []:
        if str(block.get("type") or "").strip().lower() in {"tool_use", "tool_call", "tool_result"}:
            return True
...
def _assistant_response_text(raw_content: Any) -> str:
    ...
    if isinstance(raw_content, dict):
        for key in ("text", "content", "output_text"):
            ...
            if value:
                return _assistant_response_text(value)
...
class ChatSessionState:
    ...
    conversation: PentestConversationSession | None = None
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "ChatSessionState":
        messages = [ChatMessage.from_dict(item) for item in payload.get("messages") or []]
        ...
        return cls(
            ...
            error=payload.get("error"),
            memory=LayeredConversationMemory.model_validate(payload.get("memory") or {}),
            messages=messages,
        )
...
class ChatSessionManager:
    def __init__(self, project_root: Path) -> None:
        ...
        default_config_path = str(os.getenv("AUTOSONGSHU_DEFAULT_CONFIG_PATH") or "").strip()
        if default_config_path:
            ...
            self.default_config_path = self.project_root / "configs" / "pentest.example.yaml"

        default_artifact_root = str(
            os.getenv("AUTOSONGSHU_DEFAULT_ARTIFACT_ROOT") or os.getenv("AUTOSONGSHU_ARTIFACT_ROOT_DIR") or "",
        ).strip()
        ...
        self.executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="autosongshu-chat")
    ...
    def _runtime_knowledge_search(
        ...
        normalized_ids = self.knowledge_store.filter_existing_base_ids(knowledge_base_ids)
    ...
    def _resolve_turn_knowledge_base_ids(self, linked_ids: list[str]) -> list[str]:
        normalized_linked = [str(item or "").strip() for item in linked_ids if str(item or "").strip()]
        if normalized_linked:
            existing_linked = self.knowledge_store.filter_existing_base_ids(normalized_linked)
            if existing_linked:
    ...
    def _history_message_to_agent_msg(self, message: ChatMessage) -> Msg:
    ...
    def _session_pinned_context(self, session: ChatSessionState) -> str:
        completed_user_messages = [message for message in session.messages if message.role == "user" and message.status == "completed"]
        initial_goal = completed_user_messages[0].text_content() if completed_user_messages else ""
        recent_sandbox_scripts = _extract_recent_sandbox_script_paths(session.messages)
        knowledge_base_name_map = self.knowledge_store.get_base_name_map(session.knowledge_base_ids)
        ...
        for message in completed_user_messages:
            for url in _extract_absolute_urls(message.text_content()):
                collect_url(url)
        ...
        if session.engagement_notes:
            lines.append(f"- Engagement notes: {_truncate_text(session.engagement_notes, 240)}")
        if knowledge_base_name_map:
            ...
            if ordered_names:
                ...
                lines.append("- Use `knowledge_search` when retrieval is needed; it is no longer forced on every turn.")
        if recent_sandbox_scripts:
            lines.append(f"- Recent sandbox Python scripts already in use: {', '.join(recent_sandbox_scripts)}")
            lines.append(
    ...
    def _should_auto_compact_session(
        ...
        assistant_turns = sum(1 for message in pending_messages if message.role == "assistant")
        if assistant_turns < self._compaction_min_turns(session):
            return False

        # OpenCode-style compaction is budget-driven instead of running on every turn.
    ...
    def _split_compaction_messages(
        ...
        assistant_indexes = [index for index, message in enumerate(pending_messages) if message.role == "assistant"]
        if len(assistant_indexes) <= retain_recent_turns:
            return [], pending_messages

        retain_start_index = assistant_indexes[-retain_recent_turns]
        if retain_start_index > 0 and pending_messages[retain_start_index - 1].role == "user":
            retain_start_index -= 1
        return pending_messages[:retain_start_index], pending_messages[retain_start_index:]
    ...
    def _build_memory_refresh_job(
        ...
        *,
        ...
        pending_memory_messages = self._context_history_messages(session, skip_message_ids=skip_message_ids)
        if not pending_memory_messages:
            return None
        if not self._should_auto_compact_session(session, pending_memory_messages):
            return None
        compactable_messages, _retained_messages = self._split_compaction_messages(session, pending_memory_messages)
        assistant_messages = [message for message in compactable_messages if message.role == "assistant"]
    ...
    def _execute_memory_refresh_job(self, job: _MemoryRefreshJob) -> LayeredConversationMemory:
    ...
    def _recover_interrupted_session(self, session: ChatSessionState) -> bool:
        ...
        for message in session.messages:
            ...
            if not message.text_content():
                message.content = [{"type": "output_text", "text": "上一次执行在服务重启前中断，请重新发送消息继续。"}]
    ...
    def _apply_config_overrides(
        ...
        *,
        ...
        return PentestConversationSession(config, artifact_session_name=artifact_session_name)
    ...
    def _ensure_conversation_ready(
        ...
        *,
        ...
        if session.conversation is None:
            artifact_session_name = Path(session.artifact_dir).name if session.artifact_dir else None
        ...
        conversation = session.conversation
        self._refresh_session_memory(session, conversation, skip_message_ids=skip_message_ids)
        history_messages = [
            self._history_message_to_agent_msg(message)
            for message in self._context_history_messages(session, skip_message_ids=skip_message_ids)
        ]
    ...
    def create_authorization(self, payload: AuthorizationDraft) -> dict[str, str | bool | list[str] | None]:
    ...
    def update_knowledge_base(self, knowledge_base_id: str, payload: KnowledgeBaseUpdateDraft) -> dict[str, Any]:
    ...
    def create_knowledge_document(self, knowledge_base_id: str, payload: KnowledgeDocumentDraft) -> dict[str, Any]:
        return self.knowledge_store.create_document(knowledge_base_id, payload)

    def get_knowledge_document(self, knowledge_base_id: str, document_id: str) -> dict[str, Any]:
        return self.knowledge_store.get_document(knowledge_base_id, document_id)

    def delete_knowledge_document(self, knowledge_base_id: str, document_id: str) -> None:
    ...
    def search_knowledge_chunks(
        ...
        *,
        ...
        if not normalized_ids:
            return []
        return self.knowledge_store.search_chunks(normalized_ids, query, limit=max(1, min(int(limit), 20)))

    def _resolve_target_knowledge_base_id(self, *, requested_id: str | None, linked_ids: list[str]) -> str:
        normalized_requested = str(requested_id or "").strip()
        if normalized_requested:
            existing = self.knowledge_store.filter_existing_base_ids([normalized_requested])
        ...
        if not normalized_linked:
            raise ValueError("No linked knowledge base for this chat session. Please provide knowledge_base_id.")

        existing_linked = self.knowledge_store.filter_existing_base_ids(normalized_linked)
        if not existing_linked:
            raise ValueError("Linked knowledge bases are unavailable. Please provide a valid knowledge_base_id.")
        return existing_linked[0]

    def _build_session_knowledge_transcript(self, messages: list[ChatMessage], *, max_chars: int = 18000) -> str:
        lines: list[str] = []
        for message in messages:
            if message.status != "completed" or message.role not in {"user", "assistant"}:
                continue
            text = str(message.text_content() or "").strip().replace("\r\n", "\n").replace("\r", "\n")
            if not text:
                continue
            if len(text) > 2600:
                text = f"{text[:2597]}..."
        ...
        if not lines:
            raise ValueError("No completed conversation content found for knowledge extraction.")
    ...
    def _summarize_session_experience_with_model(
        ...
        *,
        ...
        max_tokens = int(config.model.max_tokens) if config.model.max_tokens is not None else 1800
        ...
        try:
            ...
            raise RuntimeError(f"Knowledge summary model request failed: {detail}") from exc
        except Exception as exc:
            raise RuntimeError(f"Knowledge summary model request failed: {exc}") from exc
        ...
        if isinstance(choices, list) and choices:
            ...
            raw_text = _assistant_response_text(message.get("content") if isinstance(message, dict) else "")
        ...
        if parsed is not None:
            ...
            content = str(parsed.get("content") or parsed.get("content_markdown") or "").strip()
            if content:
    ...
    def _generate_knowledge_summary_from_session(
        ...
        *,
        ...
        generated_title, generated_content, model_name = self._summarize_session_experience_with_model(
    ...
    def create_knowledge_document_from_session(
        ...
        with self.lock:
            ...
            session_title = str(session.title or session.session_id or session_id).strip()
        ...
        with self.lock:
            ...
            if live_session is not None and target_knowledge_base_id not in live_session.knowledge_base_ids:
    ...
    def create_chat_session(self, payload: CreateChatSessionRequest) -> dict[str, Any]:
        allowed_hosts = [item.strip().lower() for item in payload.allowed_hosts if item.strip()]
        session = ChatSessionState(
            ...
            knowledge_base_ids=self._normalize_knowledge_base_ids(payload.knowledge_base_ids),
        )
    ...
    def _process_turn(
        ...
        def on_stream_event(event: dict[str, Any]) -> None:
            ...
            with self.lock:
                ...
                if session is None:
                    return
                assistant_message = self._find_message(session, assistant_message_id)
                assistant_message.content = merge_assistant_content(assistant_message.content, incoming)
                assistant_message.status = "in_progress"
                assistant_message.error = None
                assistant_message.updated_at = now_iso()
                session.status = "interrupting" if session.interrupt_requested else "running"
                session.error = None
                session.updated_at = assistant_message.updated_at
            flush_partial(force=_should_force_stream_flush(event))
        ...
        try:
            ...
            effective_knowledge_base_ids = self._resolve_turn_knowledge_base_ids(knowledge_base_ids)
            ...
            with self.lock:
                ...
                assistant_message.content = finalize_completed_assistant_content(assistant_message.content)
                ...
                if session.conversation is not None:
                    ...
                    if cleanup_job is None:
                        ...
                        session.cleanup_future = self.executor.submit(self._run_post_turn_memory_refresh, cleanup_job)
            ...
            with self.lock:
                ...
                if not assistant_message.content:
                    assistant_message.content = [{"type": "output_text", "text": str(exc)}]
    ...
    def shutdown(self) -> None:
        with self.lock:
            sessions = list(self.chat_sessions.values())
        cleanup_futures = [session.cleanup_future for session in sessions if session.cleanup_future is not None]
