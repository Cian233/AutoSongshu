...
_EXECUTION_CONTRACT = "\n".join(
    [
        ...
        "- When iterating on an existing sandbox payload or helper script, first inspect it with `sandbox_read_file(include_line_numbers=True)`, then prefer `sandbox_edit_file` for one precise change or `sandbox_multiedit_file` for several ordered precise changes before `sandbox_run_python(script_path=..., timeout_sec=120)`.",
...
_CONTINUATION_PROMPT = "\n".join(
    [
        ...
        "- After `sandbox_write_file`, `sandbox_edit_file`, or `sandbox_multiedit_file` changes a Python script you intend to verify, immediately follow with `sandbox_run_python(script_path=..., timeout_sec=120)` in the same turn unless a confirmed blocker prevents execution.",
